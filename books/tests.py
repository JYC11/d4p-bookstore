...
class BookTests(TestCase):
    ...
    def test_book_listing(self):
        self.assertEqual(f'{self.book.title}', 'Mistborn')
        self.assertEqual(f'{self.book.author}', 'Brandon Sanderson')
        self.assertEqual(f'{self.book.price}', '30.00')
    
    def test_book_list_vew_for_logged_in_user(self):
        self.client.login(email='reviewuser@email.com', password='testpass123')
        response = self.client.get(reverse('book_list'))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'Mistborn')
    ...
    def test_book_detail_view_with_permissions(self): # new
        ...
        self.assertContains(response, 'Mistborn')
        self.assertContains(response, 'so cool')
